...
def matrix_divided(matrix, div):
    ...
    """
    if (not isinstance(matrix, list) or matrix == [] or not all(isinstance(row, list)) for row in matrix) or not all((isinstance(values, int) or isinstance(values, float)) for values in [coloumn for row in matrix for coloumn in row]):
